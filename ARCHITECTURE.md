...
- `site/login/index.html` + `site/js/login.js` — tela de login (rota `/login/`), port em JS puro do componente canônico `design/components/MazariLogin.tsx`. Logo com animação "Fundação" (1b) tocando 1x ao abrir (reduced-motion vê pronto); formulário com validação, mostrar/ocultar senha, "manter conectado" e mensagens de erro. **Sem backend nesta fase**: envio válido mostra aviso de portal em implantação e direciona ao consultor — nenhum login é simulado. Responsivo: colunas empilham abaixo de ~980px (grid auto-fit), paddings reduzidos abaixo de 640px. Auth real entra na fase SaaS (NextAuth/API, ver spec).
